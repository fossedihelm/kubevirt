...
func validateOwnerReference(ownerRef metav1.OwnerReference, expectedOwner metav1.Object) {
	...
	ExpectWithOffset(1, ownerRef.UID).To(Equal(expectedOwner.GetUID()), err)
	ExpectWithOffset(1, ownerRef.Name).To(Equal(expectedOwner.GetName()), err)
	ExpectWithOffset(1, ownerRef.Kind).To(Equal(clonev1alpha1.VirtualMachineCloneKind.Kind), err)
	ExpectWithOffset(1, ownerRef.APIVersion).To(Equal(clonev1alpha1.VirtualMachineCloneKind.GroupVersion().String()), err)
	ExpectWithOffset(1, ownerRef.BlockOwnerDeletion).ToNot(BeNil(), err)
	ExpectWithOffset(1, *ownerRef.BlockOwnerDeletion).To(BeTrue(), err)
	ExpectWithOffset(1, ownerRef.Controller).ToNot(BeNil(), err)
	ExpectWithOffset(1, *ownerRef.Controller).To(BeTrue(), err)
}
